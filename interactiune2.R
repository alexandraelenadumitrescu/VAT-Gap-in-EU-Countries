...
# ==============================================================================
# ANALIZA CU TERMENI DE INTERACȚIUNE - EFECTE MODERATOARE
# Perfect pentru N=27: Parsimonie + Interpretabilitate Economică
# ==============================================================================

cat("=== ANALIZA EFECTELOR DE INTERACȚIUNE ===\n\n")

if(!exists("df_clean")) {
  stop("Rulați mai întâi script-ul Partea 1!")
}
...
library(sandwich)
library(car)
library(interactions)  # Pentru grafice sofisticate
library(ggplot2)
library(gridExtra)

# ==============================================================================
# 1. IPOTEZE ECONOMICE PENTRU INTERACȚIUNI
# ==============================================================================

cat("=== IPOTEZE TEORETICE ===\n\n")

cat("H1: Shadow Economy × CPI_Score (Corupție ca moderator)\n")
cat("    Ipoteză: Impactul informalității pe VAT Gap e MAI MARE în țări corupte\n")
cat("    Rațiune: Într-un mediu cu corupție ridicată, economia subterană\n")
cat("             evită mai eficient colectarea TVA.\n\n")

cat("H2: Shadow Economy × Rule_of_Law (Calitate instituțională)\n")
cat("    Ipoteză: Efectul informalității e ATENUAT în țări cu instituții puternice\n")
cat("    Rațiune: Statul de drept permite enforcement eficient.\n\n")

cat("H3: VAT_Rever × GDP_per_cap (Capacitate fiscală)\n")
cat("    Ipoteză: Veniturile din TVA reduc gap-ul mai mult în țări bogate\n")
cat("    Rațiune: Țările dezvoltate au administrație fiscală performantă.\n\n")

cat("H4: Shadow Economy × (Shadow Economy)² (Non-linearitate pătratică)\n")
cat("    Ipoteză: Relația devine mai abruptă la nivele extreme de informalitate\n")
cat("    Rațiune: Când Shadow Economy > 30%, sistemul fiscal e copleșit.\n\n")

# ==============================================================================
# 2. MODELUL BASELINE (fără interacțiuni)
# ==============================================================================

cat("=== MODELUL BASELINE (pentru comparație) ===\n\n")

m_base <- lm(VAT_Comp ~ log(ShadowEc) + VAT_Rever + CPI_Score + Rule_of_Law, 
             data=df_clean)

cat("Adj R²:", round(summary(m_base)$adj.r.squared, 4), "\n")
cat("AIC:", round(AIC(m_base), 2), "\n")
cat("BIC:", round(BIC(m_base), 2), "\n\n")

# ==============================================================================
# 3. TESTARE INTERACȚIUNI (1 câte 1)
# ==============================================================================

cat("=== TESTARE INTERACȚIUNI INDIVIDUALE ===\n\n")

# --- Interacțiune 1: Shadow Economy × CPI ---
cat("--- MODEL 1: log(ShadowEc) × CPI_Score ---\n")
m_int1 <- lm(VAT_Comp ~ log(ShadowEc) * CPI_Score + VAT_Rever + Rule_of_Law, 
             data=df_clean)

summary(m_int1)
cat("\nAdj R²:", round(summary(m_int1)$adj.r.squared, 4), "\n")
cat("AIC:", round(AIC(m_int1), 2), "\n")

# Test semnificație interacțiune (F-test)
anova_int1 <- anova(m_base, m_int1)
cat("\nF-test pentru adăugarea interacțiunii:\n")
print(anova_int1)

if(anova_int1$`Pr(>F)`[2] < 0.05) {
  cat("✓ Interacțiunea este SEMNIFICATIVĂ (p < 0.05)\n\n")
} else {
  cat("✗ Interacțiunea NU este semnificativă (p > 0.05)\n\n")
}

# --- Interacțiune 2: Shadow Economy × Rule of Law ---
cat("--- MODEL 2: log(ShadowEc) × Rule_of_Law ---\n")
m_int2 <- lm(VAT_Comp ~ log(ShadowEc) * Rule_of_Law + VAT_Rever + CPI_Score, 
             data=df_clean)

summary(m_int2)
cat("\nAdj R²:", round(summary(m_int2)$adj.r.squared, 4), "\n")
cat("AIC:", round(AIC(m_int2), 2), "\n")

anova_int2 <- anova(m_base, m_int2)
cat("\nF-test:\n")
print(anova_int2)

if(anova_int2$`Pr(>F)`[2] < 0.05) {
  cat("✓ Interacțiunea este SEMNIFICATIVĂ\n\n")
} else {
  cat("✗ Interacțiunea NU este semnificativă\n\n")
}

# --- Interacțiune 3: VAT_Rever × GDP_per_cap ---
cat("--- MODEL 3: VAT_Rever × GDP_per_cap ---\n")
m_int3 <- lm(VAT_Comp ~ log(ShadowEc) + VAT_Rever * GDP_per_cap + CPI_Score + Rule_of_Law, 
             data=df_clean)

summary(m_int3)
cat("\nAdj R²:", round(summary(m_int3)$adj.r.squared, 4), "\n")
cat("AIC:", round(AIC(m_int3), 2), "\n")

anova_int3 <- anova(m_base, m_int3)
cat("\nF-test:\n")
print(anova_int3)

if(anova_int3$`Pr(>F)`[2] < 0.05) {
  cat("✓ Interacțiunea este SEMNIFICATIVĂ\n\n")
} else {
  cat("✗ Interacțiunea NU este semnificativă\n\n")
}

# --- Interacțiune 4: Efect pătratic Shadow Economy ---
cat("--- MODEL 4: log(ShadowEc) + [log(ShadowEc)]² ---\n")
m_int4 <- lm(VAT_Comp ~ log(ShadowEc) + I(log(ShadowEc)^2) + VAT_Rever + CPI_Score + Rule_of_Law, 
             data=df_clean)

summary(m_int4)
cat("\nAdj R²:", round(summary(m_int4)$adj.r.squared, 4), "\n")
cat("AIC:", round(AIC(m_int4), 2), "\n")

anova_int4 <- anova(m_base, m_int4)
cat("\nF-test:\n")
print(anova_int4)

if(anova_int4$`Pr(>F)`[2] < 0.05) {
  cat("✓ Termenul pătratic este SEMNIFICATIV\n\n")
} else {
...
comparison <- data.frame(
  ...
  Formula = c("Additiv",
              ...
              "VAT_Rev × GDP",
              "log(Shadow)²"),
  Adj_R2 = c(summary(m_base)$adj.r.squared,
             summary(m_int1)$adj.r.squared,
             summary(m_int2)$adj.r.squared,
             summary(m_int3)$adj.r.squared,
             summary(m_int4)$adj.r.squared),
  AIC = c(AIC(m_base), AIC(m_int1), AIC(m_int2), AIC(m_int3), AIC(m_int4)),
  BIC = c(BIC(m_base), BIC(m_int1), BIC(m_int2), BIC(m_int3), BIC(m_int4)),
  N_params = c(5, 6, 6, 7, 6),
  F_test_p = c(NA, 
               anova_int1$`Pr(>F)`[2],
               anova_int2$`Pr(>F)`[2],
               anova_int3$`Pr(>F)`[2],
...
comparison[,3:7] <- round(comparison[,3:7], 4)
print(comparison)

# Identificare model optim
best_model_row <- comparison %>% 
  filter(F_test_p < 0.05 | is.na(F_test_p)) %>%
  arrange(AIC) %>% 
  head(1)

cat("\n🏆 MODEL OPTIM (AIC):", best_model_row$Model, "\n")
cat("   Adj R² =", best_model_row$Adj_R2, "| AIC =", best_model_row$AIC, "\n\n")

# Selecție model pentru analiză detaliată
if(best_model_row$Model == "Shadow × CPI") {
  best_interaction_model <- m_int1
  interaction_type <- "log(ShadowEc) × CPI_Score"
} else if(best_model_row$Model == "Shadow × RuleOfLaw") {
  best_interaction_model <- m_int2
  interaction_type <- "log(ShadowEc) × Rule_of_Law"
} else if(best_model_row$Model == "VAT_Rev × GDP") {
  best_interaction_model <- m_int3
  interaction_type <- "VAT_Rever × GDP_per_cap"
} else if(best_model_row$Model == "Shadow²") {
  best_interaction_model <- m_int4
  interaction_type <- "Quadratic"
} else {
  best_interaction_model <- m_base
  interaction_type <- "None"
}

# ==============================================================================
# 5. VIZUALIZARE INTERACȚIUNI (GRAFICE PROFESIONALE)
# ==============================================================================

cat("=== VIZUALIZARE EFECTE DE INTERACȚIUNE ===\n\n")

# --- GRAFIC 1: Interaction Plot pentru Shadow × CPI ---
if(anova_int1$`Pr(>F)`[2] < 0.10) {  # Pragul relaxat pentru N mic
  
  cat("Generare grafic: Shadow Economy × CPI Score...\n")
  
  # Categorisire CPI în tercile
  df_clean$CPI_Category <- cut(df_clean$CPI_Score, 
                               breaks=quantile(df_clean$CPI_Score, c(0, 0.33, 0.66, 1)),
                               labels=c("Corupție Ridicată", "Corupție Medie", "Corupție Scăzută"),
                               include.lowest=TRUE)
  
  # Predicții pentru plotting
  shadow_seq <- seq(min(df_clean$ShadowEc), max(df_clean$ShadowEc), length.out=50)
  cpi_levels <- quantile(df_clean$CPI_Score, c(0.1, 0.5, 0.9))
  
  pred_data <- expand.grid(
    ShadowEc = shadow_seq,
    CPI_Score = cpi_levels,
    VAT_Rever = mean(df_clean$VAT_Rever),
    Rule_of_Law = mean(df_clean$Rule_of_Law)
  )
  
  pred_data$VAT_Gap <- predict(m_int1, newdata=pred_data)
  pred_data$CPI_Label <- factor(pred_data$CPI_Score,
                                labels=c("CPI=45 (Bulgaria nivel)", 
                                         "CPI=61 (Mediana UE)", 
                                         "CPI=82 (Danemarca nivel)"))
  
  p1 <- ggplot() +
    geom_point(data=df_clean, aes(x=ShadowEc, y=VAT_Comp, color=CPI_Category),
               size=3, alpha=0.6) +
    geom_line(data=pred_data, aes(x=ShadowEc, y=VAT_Gap, color=CPI_Label, group=CPI_Label),
              linewidth=1.5) +
    scale_color_manual(values=c("#d73027", "#fee090", "#1a9850", 
                                "#d73027", "#fee090", "#1a9850"),
                       name="") +
    labs(title="Efect Moderator: Corupția amplifică impactul Shadow Economy",
         subtitle="Țările corupte (CPI scăzut) sunt mai vulnerabile la informalitate",
  ...
  ggsave("interaction_shadow_cpi.png", p1, width=12, height=8, dpi=300)
  cat("✓ Grafic salvat: interaction_shadow_cpi.png\n\n")
}

# --- GRAFIC 2: Marginal Effects Plot ---
if(anova_int1$`Pr(>F)`[2] < 0.10) {
  
  cat("Generare grafic: Efecte Marginale...\n")
  
  # Efectul marginal al Shadow Economy la diferite nivele de CPI
  cpi_grid <- seq(min(df_clean$CPI_Score), max(df_clean$CPI_Score), length.out=50)
  
  # Calcul manual: ∂VAT_Gap/∂log(Shadow) = β_Shadow + β_interaction * CPI
  beta_shadow <- coef(m_int1)["log(ShadowEc)"]
  beta_interaction <- coef(m_int1)["log(ShadowEc):CPI_Score"]
  
  marginal_effects <- data.frame(
    CPI_Score = cpi_grid,
    Marginal_Effect = beta_shadow + beta_interaction * cpi_grid
  )
  
  # Standard errors (aproximativ)
  vcov_mat <- vcov(m_int1)
  se_grid <- sqrt(vcov_mat["log(ShadowEc)", "log(ShadowEc)"] + 
                    cpi_grid^2 * vcov_mat["log(ShadowEc):CPI_Score", "log(ShadowEc):CPI_Score"] +
                    2 * cpi_grid * vcov_mat["log(ShadowEc)", "log(ShadowEc):CPI_Score"])
  
  marginal_effects$Lower <- marginal_effects$Marginal_Effect - 1.96 * se_grid
  marginal_effects$Upper <- marginal_effects$Marginal_Effect + 1.96 * se_grid
  
  p2 <- ggplot(marginal_effects, aes(x=CPI_Score, y=Marginal_Effect)) +
    geom_line(linewidth=1.5, color="darkblue") +
    geom_ribbon(aes(ymin=Lower, ymax=Upper), alpha=0.2, fill="darkblue") +
    geom_hline(yintercept=0, linetype="dashed", color="red", linewidth=1) +
    labs(title="Efectul Marginal al Shadow Economy în funcție de CPI",
         subtitle="Cum variază impactul informalității pe măsură ce corupția scade",
         x="CPI Score (→ corupție mai scăzută)",
         y="∂(VAT Gap) / ∂log(Shadow Economy)") +
    theme_minimal(base_size=13) +
    theme(plot.title=element_text(face="bold"))
  
  ggsave("marginal_effects_cpi.png", p2, width=12, height=7, dpi=300)
  cat("✓ Grafic salvat: marginal_effects_cpi.png\n\n")
}

# --- GRAFIC 3: Heatmap (dacă există interacțiune continuă × continuă) ---
if(anova_int3$`Pr(>F)`[2] < 0.10) {
  
  cat("Generare grafic: Heatmap VAT_Rev × GDP...\n")
  
  # Grid pentru predicții
  vat_seq <- seq(min(df_clean$VAT_Rever), max(df_clean$VAT_Rever), length.out=30)
  gdp_seq <- seq(min(df_clean$GDP_per_cap), max(df_clean$GDP_per_cap), length.out=30)
  
  heatmap_data <- expand.grid(
    VAT_Rever = vat_seq,
    GDP_per_cap = gdp_seq,
    ShadowEc = mean(df_clean$ShadowEc),
    CPI_Score = mean(df_clean$CPI_Score),
    Rule_of_Law = mean(df_clean$Rule_of_Law)
  )
  
  heatmap_data$VAT_Gap <- predict(m_int3, newdata=heatmap_data)
  
  p3 <- ggplot(heatmap_data, aes(x=VAT_Rever, y=GDP_per_cap, fill=VAT_Gap)) +
    geom_tile() +
    geom_contour(aes(z=VAT_Gap), color="white", alpha=0.5) +
    scale_fill_gradient2(low="darkgreen", mid="yellow", high="darkred",
                         midpoint=median(heatmap_data$VAT_Gap),
                         name="VAT Gap (%)") +
    labs(title="Interacțiune: VAT Revenues × GDP per Capita",
         subtitle="Efectul combinat al capacității fiscale și veniturilor efective",
         x="VAT Revenues (% GDP)",
         y="GDP per Capita (€)") +
    theme_minimal(base_size=13)
  
  ggsave("heatmap_vat_gdp.png", p3, width=12, height=8, dpi=300)
  cat("✓ Grafic salvat: heatmap_vat_gdp.png\n\n")
}

# ==============================================================================
# 6. INTERPRETARE ECONOMICĂ DETALIATĂ
# ==============================================================================

cat("=== INTERPRETARE ECONOMICĂ (MODEL OPTIM) ===\n\n")

if(interaction_type == "log(ShadowEc) × CPI_Score") {
  
  cat("MODEL FINAL: VAT_Gap = β₀ + β₁·log(Shadow) + β₂·CPI + β₃·[log(Shadow)×CPI] + ...\n\n")
  
  coefs <- coef(m_int1)
  cat("Coeficienți:\n")
  cat("  β₁ (log(Shadow)):", round(coefs["log(ShadowEc)"], 3), "\n")
  cat("  β₂ (CPI):", round(coefs["CPI_Score"], 3), "\n")
  cat("  β₃ (Interacțiune):", round(coefs["log(ShadowEc):CPI_Score"], 3), "\n\n")
  
  cat("📊 INTERPRETARE CORECTĂ:\n\n")
  cat("Efectul marginal al Shadow Economy DEPINDE de nivelul CPI:\n\n")
  cat("  ∂(VAT_Gap) / ∂log(Shadow) = β₁ + β₃ × CPI\n")
  cat("                             = ", round(coefs["log(ShadowEc)"], 3), 
      " + ", round(coefs["log(ShadowEc):CPI_Score"], 3), " × CPI\n\n", sep="")
  
  # Calcul efecte la valori specifice
  cpi_low <- 45  # Bulgaria
  cpi_mid <- 61  # Mediana UE
  cpi_high <- 82 # Danemarca
  
  effect_low <- coefs["log(ShadowEc)"] + coefs["log(ShadowEc):CPI_Score"] * cpi_low
  effect_mid <- coefs["log(ShadowEc)"] + coefs["log(ShadowEc):CPI_Score"] * cpi_mid
  effect_high <- coefs["log(ShadowEc)"] + coefs["log(ShadowEc):CPI_Score"] * cpi_high
  
  cat("EXEMPLE CONCRETE:\n\n")
  cat("1. Țară CORUPTĂ (CPI = 45, ex: Bulgaria):\n")
  cat("   Efect = ", round(effect_low, 3), "\n", sep="")
  cat("   → 1% ↑ în Shadow Economy → +", round(effect_low/100, 4), " pp în VAT Gap\n\n", sep="")
  
  cat("2. Țară MEDIE (CPI = 61, mediana UE):\n")
  cat("   Efect = ", round(effect_mid, 3), "\n", sep="")
  cat("   → 1% ↑ în Shadow Economy → +", round(effect_mid/100, 4), " pp în VAT Gap\n\n", sep="")
  
  cat("3. Țară CURATĂ (CPI = 82, ex: Danemarca):\n")
  cat("   Efect = ", round(effect_high, 3), "\n", sep="")
  cat("   → 1% ↑ în Shadow Economy → +", round(effect_high/100, 4), " pp în VAT Gap\n\n", sep="")
  
  cat("💡 CONCLUZIE ECONOMICĂ:\n")
  if(coefs["log(ShadowEc):CPI_Score"] < 0) {
    cat("   Corupția AMPLIFICĂ impactul negativ al informalității!\n")
    cat("   În țări cu instituții slabe, economia subterană generează gap-uri\n")
    cat("   disproporționat de mari comparativ cu țările cu CPI ridicat.\n\n")
  } else {
    cat("   Corupția ATENUEAZĂ impactul informalității (rezultat contra-intuitiv).\n")
    cat("   Posibilă explicație: în țări corupte, VAT-ul e deja subcolectat\n")
    cat("   chiar în sectorul formal → Shadow Ec are efect marginal redus.\n\n")
  }
  
} else if(interaction_type == "Quadratic") {
  
  cat("MODEL FINAL: VAT_Gap = β₀ + β₁·log(Shadow) + β₂·[log(Shadow)]² + ...\n\n")
  
  coefs <- coef(m_int4)
  cat("Coeficienți:\n")
  cat("  β₁ (log(Shadow)):", round(coefs["log(ShadowEc)"], 3), "\n")
  cat("  β₂ ([log(Shadow)]²):", round(coefs["I(log(ShadowEc)^2)"], 3), "\n\n")
  
  cat("📊 INTERPRETARE:\n\n")
  cat("Relația este PĂTRATICĂ → efectul se accelerează/decelerează.\n\n")
  
  if(coefs["I(log(ShadowEc)^2)"] > 0) {
    cat("  β₂ > 0 → Curbă CONVEXĂ (efectul crește exponențial)\n")
    cat("  Semnificație: La nivele MARI de informalitate (>30%), fiecare\n")
    cat("  procent adițional produce daune disproporționate.\n\n")
    cat("  Analogie: 'Ultima picătură care umple paharul' - sistemul fiscal\n")
    cat("  colapsează când Shadow Economy depășește un prag critic.\n\n")
  } else {
    cat("  β₂ < 0 → Curbă CONCAVĂ (efectul se saturează)\n")
    cat("  Semnificație: Beneficiile marginale ale reducerii Shadow Economy\n")
    cat("  scad la nivele deja ridicate de informalitate.\n\n")
    cat("  Analogie: 'Fructele ușoare' - e mai eficient să combați informalitatea\n")
    cat("  în țări cu Shadow Ec moderat (15-25%) decât în cele cu >30%.\n\n")
  }
  
} else {
  cat("Modelul baseline (fără interacțiuni) rămâne optim.\n")
  cat("Nicio interacțiune testată nu îmbunătățește semnificativ fit-ul.\n\n")
}

# ==============================================================================
# 7. TESTARE ROBUSTEȚE INTERACȚIUNE
# ==============================================================================

cat("=== TESTARE ROBUSTEȚE (Robust SE) ===\n\n")

if(interaction_type != "None") {
  robust_results <- coeftest(best_interaction_model, 
                             vcov=vcovHC(best_interaction_model, type="HC3"))
  
  cat("Coeficienți cu Robust Standard Errors (HC3):\n")
  print(robust_results)
  
  # Verificare dacă interacțiunea rămâne semnificativă
  interaction_coef_name <- grep(":", names(coef(best_interaction_model)), value=TRUE)[1]
  if(!is.na(interaction_coef_name)) {
    p_robust <- robust_results[interaction_coef_name, "Pr(>|t|)"]
    cat("\nInteracțiunea cu Robust SE: p =", round(p_robust, 4), "\n")
    if(p_robust < 0.05) {
      cat("✓ Interacțiunea rămâne SEMNIFICATIVĂ cu robust SE\n\n")
    } else {
      cat("⚠️ Interacțiunea își pierde semnificația cu robust SE\n")
      cat("   → Posibil artefact cauzat de outlieri sau heteroscedasticitate\n\n")
    }
  }
}
...
cat("\n\n===============================================\n")
cat("✅ ANALIZA INTERACȚIUNILOR COMPLETĂ\n")
cat("===============================================\n\n")

cat("📊 REZUMAT:\n")
cat("• Modele testate:", nrow(comparison), "\n")
cat("• Model optim:", best_model_row$Model, "(", best_model_row$Formula, ")\n")
cat("• Îmbunătățire Adj R²:", 
    round((best_model_row$Adj_R2 - comparison$Adj_R2[1]) * 100, 2), "pp\n")
cat("• Reducere AIC:", round(comparison$AIC[1] - best_model_row$AIC, 2), "\n\n")

cat("📁 FIȘIERE GENERATE:\n")
if(file.exists("interaction_shadow_cpi.png")) cat("  • interaction_shadow_cpi.png\n")
if(file.exists("marginal_effects_cpi.png")) cat("  • marginal_effects_cpi.png\n")
if(file.exists("heatmap_vat_gdp.png")) cat("  • heatmap_vat_gdp.png\n")

cat("\n💡 RECOMANDĂRI RAPORT:\n\n")
cat("1. Includeți OBLIGATORIU interaction plot-ul în secțiunea de rezultate\n")
cat("2. Interpretați efectele marginale la valori LOW/MEDIUM/HIGH ale moderatorului\n")
cat("3. Discutați implicațiile de politică economică:\n")
cat("   'Reducerea corupției nu doar scade direct VAT Gap-ul, ci AMPLIFICĂ\n")
cat("    și eficiența măsurilor de combatere a economiei subterane.'\n")
cat("4. Menționați limitările: N mic → CI largi pentru coeficientul de interacțiune\n")
cat("5. Comparați cu literatura: efecte moderatoare similare în Keen (2010)?\n\n")

cat("===============================================\n")
cat("GATA! Aveți analiza de interacțiune COMPLETĂ.\n")
cat("===============================================\n")
